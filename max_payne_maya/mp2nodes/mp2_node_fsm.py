...
from max_payne_maya.mp2nodes.mp2_node_ids import MP2_FSM_NODE_NAME, MP2_FSM_NODE_ID, MP2_FSM_NODE_ID, MP2_FSM_NODE_NAME


class MP2NodeFSM(OpenMayaUI.MPxLocatorNode):
    ...
    def create_fsm_attr():
        ...
        MP2NodeFSM.FSMAttr = fsm_attr.create("na_fsm", "na_fsm", OpenMaya.MFnData.kString, fsm_string)
        fsm_attr.hidden = False
        fsm_attr.keyable = False
        fsm_attr.writable = True
        fsm_attr.storable = True
        MP2NodeFSM.addAttribute(MP2NodeFSM.FSMAttr)

    @staticmethod
    def initializer():
        MP2NodeFSM.initializeBaseAttributes()
        MP2NodeFSM.create_fsm_attr()

    @staticmethod
    def initializeBaseAttributes():
        hidden_attr = OpenMaya.MFnNumericAttribute()
        MP2NodeFSM.HiddenAttr = hidden_attr.create("na_hidden", "na_hidden", OpenMaya.MFnNumericData.kBoolean, 0)
        hidden_attr.hidden = False
        hidden_attr.keyable = False
        hidden_attr.writable = True
        hidden_attr.storable = True
        MP2NodeFSM.addAttribute(MP2NodeFSM.HiddenAttr)

        exclude_from_game_attr = OpenMaya.MFnNumericAttribute()
        MP2NodeFSM.ExcludeFromGameAttr = exclude_from_game_attr.create("na_excludeFromGame", "na_excludeFromGame",
        ...
        MP2NodeFSM.addAttribute(MP2NodeFSM.ExcludeFromGameAttr)

        exclude_from_lighting_attr = OpenMaya.MFnNumericAttribute()
        MP2NodeFSM.ExcludeFromLightingAttr = exclude_from_lighting_attr.create("na_excludeFromLighting",
        ...
        MP2NodeFSM.addAttribute(MP2NodeFSM.ExcludeFromLightingAttr)

        enable_export_regrouping_attr = OpenMaya.MFnNumericAttribute()
        MP2NodeFSM.EnableExportRegroupingAttr = enable_export_regrouping_attr.create("na_enableExportRegrouping",
        ...
        MP2NodeFSM.addAttribute(MP2NodeFSM.EnableExportRegroupingAttr)
...
class MP2NodeFSMDrawData(OpenMaya.MUserData):
    def __init__(self):
...
class MP2NodeFSMDrawOverride(OpenMayaRender.MPxDrawOverride):

    @staticmethod
    def creator(obj):
        return MP2NodeFSMDrawOverride(obj)
    ...
    def prepareForDraw(self, objPath, cameraPath, frameContext, oldData):
        data = oldData if isinstance(oldData, MP2NodeFSMDrawData) else MP2NodeFSMDrawData()
        return data
    ...
    def addUIDrawables(self, objPath, drawManager, frameContext, data):
        if not isinstance(data, MP2NodeFSMDrawData):
            return
        ...
        drawManager.text(data.text_center, 'MP2_FSM', OpenMayaRender.MUIDrawManager.kCenter)
